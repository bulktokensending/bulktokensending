...
import BulkTokenSendingAbi from "../abis/BulkTokenSending"
...
class TxStore {
    ...
    if(new BN(this.tokenStore.totalBalance).gt(new BN(this.tokenStore.allowance))){
      ...
      const interval = setInterval(() => {
        ...
        if(this.approval){
          ...
          if(this.txs[index] && this.txs[index].status === 'error'){
            clearInterval(interval);
            swal("Error!", this.txs[index].name, 'error')
            console.error(this.txs[index])
          }
        }
      }, 3000)
  ...
  async _approve(){
    ...
    try{
      return token.methods['approve(address,uint256)'](this.tokenStore.proxyBulkSenderAddress, this.tokenStore.totalBalanceWithDecimals)
      .send({from: this.web3Store.defaultAccount, gasPrice: this.gasPriceStore.standardInHex})
      .on('transactionHash', (hash) => {
        this.approval = hash
        this.txHashToIndex[hash] = index;
        this.txs[index] = {status: 'pending', name: `BulkTokenSending Approval to spend ${this.tokenStore.totalBalance} ${this.tokenStore.tokenSymbol}`, hash}
  ...
  async _multisend({slice, addPerTx}) {
    ...
    let {addresses_to_send, balances_to_send, proxyBulkSenderAddress, currentFee} =  this.tokenStore;
    ...
    if(token_address === "0x000000000000000000000000000000000000bEEF"){
      ...
      const totalInEth = Web3Utils.fromWei(totalInWei.toFixed())
    ...
    const web3 = this.web3Store.web3;
    const BulkTokenSending = new web3.eth.Contract(BulkTokenSendingAbi, proxyBulkSenderAddress);

    try {
      let encodedData = await BulkTokenSending.methods.multisendToken(token_address, addresses_to_send, balances_to_send).encodeABI({from: this.web3Store.defaultAccount})
      let gas = 7000000;
      try {
        gas = await web3.eth.estimateGas({
            from: this.web3Store.defaultAccount,
            data: encodedData,
            value: Web3Utils.toHex(Web3Utils.toWei(ethValue.toFixed())),
            to: proxyBulkSenderAddress
        })
      } catch(e) {
        console.log("estimation Gas error", e);
      }

      BulkTokenSending.methods.multisendToken(token_address, addresses_to_send, balances_to_send)
      .send({
        from: this.web3Store.defaultAccount,
