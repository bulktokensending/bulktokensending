...
class TokenStore {
  ...
  @observable arrayLimit = 100
  @observable errors = []
  @observable dublicates = []
  proxyBulkSenderAddress = process.env.REACT_APP_PROXY_ADDRESS
  ...
  async getAllowance() {
    try {
      const web3 = this.web3Store.web3;
      const token = new web3.eth.Contract(ERC20ABI, this.tokenAddress);
      const allowance = await token.methods.allowance(this.web3Store.defaultAccount, this.proxyBulkSenderAddress).call();
  ...
  async getCurrentFee(){
    try {
      this.web3Store.getWeb3Promise.then(async () => {
        const web3 = this.web3Store.web3;
        const bulksender = new web3.eth.Contract(BulkTokenSendingABI, this.proxyBulkSenderAddress);
        const currentFee = await bulksender.methods.currentFee(this.web3Store.defaultAccount).call();
  ...
  async getArrayLimit(){
    try {
      await this.web3Store.getWeb3Promise.then(async () => {
        const web3 = this.web3Store.web3;
        const bulksender = new web3.eth.Contract(BulkTokenSendingABI, this.proxyBulkSenderAddress);
        this.arrayLimit = await bulksender.methods.arrayLimit().call();
        console.log("arrayLimit", this.arrayLimit);
  ...
  @computed get totalCostInEth(){
    ...
    if(Number(this.currentFee).valueOf() !== 0) {
